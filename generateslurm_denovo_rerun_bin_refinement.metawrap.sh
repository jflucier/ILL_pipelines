#!/bin/bash

set -e

echo "load and valdiate env"
# load and valdiate env
export EXE_PATH=$(dirname "$0")

if [ -z ${1+x} ]; then
    echo "Please provide a configuration file. See ${EXE_PATH}/my.example.config for an example."
    exit 1
fi

export CONF_PARAMETERS=$1
source $CONF_PARAMETERS

${EXE_PATH}/scripts/global.checkenv.sh
${EXE_PATH}/scripts/denovo_assembly.metawrap.checkenv.sh

mkdir -p ${OUTPUT_PATH}/${ASSEMBLY_OUTPUT_NAME}/logs

echo "outputting denovo bin_refinement slurm script to ${OUTPUT_PATH}/${ASSEMBLY_OUTPUT_NAME}/denovo_bin_refinement.metawrap.slurm.sh"

echo '#!/bin/bash' > ${OUTPUT_PATH}/${ASSEMBLY_OUTPUT_NAME}/denovo_bin_refinement.metawrap.slurm.sh
echo '
#SBATCH --mail-type=END,FAIL
#SBATCH -D '${OUTPUT_PATH}'
#SBATCH -o '${OUTPUT_PATH}'/'${ASSEMBLY_OUTPUT_NAME}'/logs/denovo_bin_refinement-%A_%a.slurm.out
#SBATCH --time='${ASSEMBLY_SLURM_WALLTIME}'
#SBATCH --mem='${ASSEMBLY_SLURM_MEMORY}'
#SBATCH --mail-user='${SLURM_JOB_EMAIL}'
#SBATCH -N 1
#SBATCH -n '${ASSEMBLY_SLURM_NBR_THREADS}'
#SBATCH -A '${SLURM_ALLOCATION}'
#SBATCH -J bin_refinement

newgrp def-ilafores
echo "loading env"
export MUGQIC_INSTALL_HOME=/cvmfs/soft.mugqic/CentOS6
module use $MUGQIC_INSTALL_HOME/modulefiles

bash '${EXE_PATH}'/scripts/denovo_bin_refinement.metawrap.sh \
'$CONF_PARAMETERS' \
$SLURM_TMPDIR \
$SLURM_ARRAY_TASK_ID

' >> ${OUTPUT_PATH}/${ASSEMBLY_OUTPUT_NAME}/denovo_bin_refinement.metawrap.slurm.sh

echo "To submit to slurm, execute the following command:"
read sample_nbr f <<< $(wc -l ${ASSEMBLY_SAMPLE_LIST_TSV})
echo "sbatch --array=1-$sample_nbr ${OUTPUT_PATH}/${ASSEMBLY_OUTPUT_NAME}/denovo_bin_refinement.metawrap.slurm.sh"
